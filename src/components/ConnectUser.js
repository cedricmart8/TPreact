...
var verif = "";
var Dialog = require("react-mdl").Dialog;
...
class ConnectUser extends React.Component {
  ...
  handleSubmit(event){
    ...
    fetch('https://messy.now.sh/authenticate', {
      ...
      if(verif === false){
        this.setState({erreur : <p style={{backgroundColor:"#FF5252", fontWeight:"bold", boxShadow:" 0 10px 20px rgba(0,0,0,0.19)"}}>Wrong Login/Password</p>})
      };
      return results.json();
    }).then(data => {
      this.props.onUserLogged(data);
    })
  }
...
render(){
  ...
  return (
    <div style={{width:"320px", margin:"30px", padding:"10px", textAlign:"center", backgroundColor:"white",  border:"2px", boxShadow:" 0 10px 20px rgba(0,0,0,0.19)"}}>
      {this.state.erreur}
      <h3>Connexion</h3>
